...
# --- Parameters ---
GRID_SIZE_REGULAR = 20   # Coarser input grid for kernel method
GRID_SIZE_OUTPUT = 15    # Output grid for animation steps (determines number of frames)
...
kernel_pixel_radius = KERNEL_SIZE_DISCRETE // 2 # Radius of the discrete kernel in pixels
...
def animate(frame_num):
    ...
    focal_x_world = x_out[col_out_idx]
    ...
    # ---- MODIFIED REVEAL LOGIC FOR KERNEL METHOD ----
    # Define the slice of the regular grid covered by the kernel
    row_start_reveal = max(0, kernel_center_row_idx_orig - kernel_pixel_radius)
    row_end_reveal = min(GRID_SIZE_REGULAR, kernel_center_row_idx_orig + kernel_pixel_radius + 1)
    col_start_reveal = max(0, kernel_center_col_idx_orig - kernel_pixel_radius)
    col_end_reveal = min(GRID_SIZE_REGULAR, kernel_center_col_idx_orig + kernel_pixel_radius + 1)
    
    # Reveal the part of the pre-calculated smoothed data under the current kernel position
    displayed_smoothed_regular[row_start_reveal:row_end_reveal, col_start_reveal:col_end_reveal] = \
        full_smoothed_regular_data[row_start_reveal:row_end_reveal, col_start_reveal:col_end_reveal]
...
gif_filename = 'gaussian_filter_comparison_solid_kernel.gif'
